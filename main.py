...
parser.add_argument("--dataset", type=str, default="mip1", help="choose dataset")
...
def train():
    ...
    step1 = 0
    step2 = 0
    for epoch in range(opt.epoch, opt.n_epochs):  ## for epoch in (0, 200)
        for i, batch in enumerate(train_dataloader):  ## batch is a dict, batch['AB']:(1, 3, 256, 256), batch['B']:(1, 3, 256, 256)
            step1 += 1
            ...
            ## GAN loss
            loss_GAN = criterion_GAN(discriminator(fake_B), valid) + criterion_GAN(discriminator(fake_A), valid)

            # Cycle loss 循环一致性损失
            recov_A = generator(fake_B, real_AB)
...
####################################################################
        if epoch >= 20:
            for i, dev_data in enumerate(dev_dataloader, 0):
                ...
                real_A = dev_crimer_image.cuda()
                real_B = dev_cocrimer_image.cuda()
                real_AB = dev_morph_image.cuda()
                ...
                ## GAN loss
                loss_GAN = criterion_GAN(discriminator(fake_A), valid)
